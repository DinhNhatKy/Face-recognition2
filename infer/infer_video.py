...
def infer_camera(min_face_area=config['min_face_area'], bbox_threshold=config['bbox_threshold'], required_images=config['required_images']):
...
def check_validation(input, embeddings, image2class, idx_to_class, recogn_model, is_anti_spoof= config['is_anti_spoof'], validation_threshold= config['validation_threshold'], is_vote= config['is_vote'], distance_mode = config['distance_mode'], anti_spoof_threshold= config['anti_spoof_threshold']):
